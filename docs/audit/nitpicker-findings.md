...
#### [N-166] snacks `image` snack undocumented `imagemagick` dependency
Fixed: 2026-06-08
Notes: Added `-- PDF/SVG/math preview additionally requires imagemagick in $PATH.`
comment to the `image = { enabled = true }` block in `config/nvim/init.lua`.

#### [N-162] blink.cmp forced to Lua fuzzy implementation
Fixed: 2026-06-08
Notes: Removed `fuzzy = { implementation = 'lua' }` and its comment from
`config/nvim/init.lua`. blink.cmp now uses the upstream default
`prefer_rust_with_warning`, which uses the faster Rust binary where available.

#### [N-163] `branch = 'main'` for nvim-treesitter
Fixed: 2026-06-08
Notes: Already resolved before this pass — nvim-treesitter was replaced by
`arborist.nvim` (Neovim 0.12+ built-in treesitter manager). No `branch = 'main'`
entry exists in the current `vim.pack.add` block.

#### [N-164] `version = vim.version.range '*'` inconsistency
Fixed: 2026-06-08
Notes: Added a three-line comment above `vim.pack.add {}` in `config/nvim/init.lua`
explaining that `version = vim.version.range '*'` is set only on plugins that publish
semver-tagged releases (blink.cmp and snacks.nvim). Remaining plugins have no
version-constrainable tags and track their default branch.

#### [N-137] Stale `config/zsh/completion/_dfm` shadows generated completion
Fixed: 2026-06-08
Notes: Already resolved before this pass — `config/zsh/completion/_dfm` no longer
exists in the repository. Directory contains only `_cheat`, `_git-profile`, `_solo2`.

#### [N-138] `dfm help` spawns ≈11 subprocesses
Fixed: 2026-06-08
Notes: Accepted advisory — no code change. Per-subprocess cost was already reduced by
the cheap-bootstrap split (expensive `shared.sh` sourcing gated off the listing path).
Subprocess count (≈11) is the inherent cost of the per-file menu design and acceptable
on an interactive non-hot path.
